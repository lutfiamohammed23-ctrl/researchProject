...
//         // Also save the first role as "role" for convenience
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://192.168.18.198:8080/api/auth/verify-otp",
        { usernameOrEmail, otp }
      );
      ...
      // --- IMPORTANT PART ---
      // Backend already returns roles as: ["APPLICANT"]
      if (Array.isArray(user.roles)) {
        localStorage.setItem("roles", JSON.stringify(user.roles));
        console.log("✅ Stored roles:", user.roles);
      } else {
        localStorage.setItem("roles", JSON.stringify([]));
        console.warn("⚠️ Roles missing, saved empty array");
      }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: "30%", minWidth: "320px", borderRadius: "1rem" }}
      >
        <div className="card-body">
          <h5 className="card-title text-center pb-0 fs-4">Verify OTP</h5>
          <p className="text-center small">Enter the OTP sent to your email</p>
